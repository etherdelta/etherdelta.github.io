...
$('#buy_cross_modal').on('show.bs.modal', function(e) {
  ...
  $('#buy_cross_desc').html('Sell order: '+bundle.utility.weiToEth(Math.abs(Number(order.availableVolume)))+' @ '+Number(order.price).toFixed(3));
});
$('#sell_cross_modal').on('show.bs.modal', function(e) {
  var order = $(e.relatedTarget).data('order');
  $('#sell_cross_order').val(JSON.stringify(order.order));
  $('#sell_cross_desc').html('Buy order: '+bundle.utility.weiToEth(Math.abs(Number(order.availableVolume)))+' @ '+Number(order.price).toFixed(3));
});
$(function () {
